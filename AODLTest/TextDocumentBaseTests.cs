...
namespace AODLTest
{
    ...
    public class TextDocumentBaseTests
    {
        ...
        public void NumberedListTest()
        {
            ...
            NUnit.Framework.Assert.IsNotNull(li.Node, "Node object must exist!");
            NUnit.Framework.Assert.IsNotNull(li.Style, "Style object must exist!");
            NUnit.Framework.Assert.IsNotNull(li.ListStyle.ListlevelStyles, "ListLevelStyleCollection must exist!");
            NUnit.Framework.Assert.IsTrue(li.ListStyle.ListlevelStyles.Count == 10, "Must exist exactly 10 ListLevelStyle objects!");
            NUnit.Framework.Assert.IsNotNull(li.ListStyle.ListlevelStyles[1].ListLevelProperties, "ListLevelProperties object must exist!");
        }
        ...
        public void ListItemTest()
        {
            ...
            NUnit.Framework.Assert.IsNotNull(lit.Content, "Content object must exist!");
            //Create a paragraph	
            Paragraph paragraph = ParagraphBuilder.CreateStandardTextParagraph(document);
            //Add some text
        ...
        public void ManipulateACommonStyle()
        {
            TextDocument document = new TextDocument();
            document.New();
            NUnit.Framework.Assert.IsTrue(document.CommonStyles.Count > 0, "Common style resp. style templates must be loaded");
            //Find a Header template
            IStyle style = document.CommonStyles.GetStyleByName("Heading_20_1");
            NUnit.Framework.Assert.IsNotNull(style, "Style with name Heading_20_1 must exist");
            NUnit.Framework.Assert.IsTrue(style is ParagraphStyle, "style must be a ParagraphStyle");
        ...
        public void InsertAndRemoveTest()
        {
            ...
            NUnit.Framework.Assert.IsNotEmpty(paragraph.TextContent, "Must be contain one element.");
            //Remove the simple text
            paragraph.TextContent.Remove(simpleText);
            NUnit.Framework.Assert.IsEmpty(paragraph.TextContent, "Must be empty");
            NUnit.Framework.Assert.IsTrue(paragraph.Node.InnerXml.Length == 0, "Node from simple text must be removed.");
        }
        ...
        public void TestCloning()
        {
            ...
            NUnit.Framework.Assert.AreNotSame(paragraph.Node, paragraphClone.Node, "Should be cloned and not equal.");
            NUnit.Framework.Assert.AreEqual(paragraph.TextContent[0].GetType(), paragraphClone.TextContent[0].GetType(), "Should be cloned and equal.");
            ...
            NUnit.Framework.Assert.AreNotSame(paragraphClone2.Node, paragraphClone.Node, "Should be cloned and not equal.");
            NUnit.Framework.Assert.AreEqual(paragraphClone2.TextContent[0].GetType(), paragraphClone.TextContent[0].GetType(), "Should be cloned and equal.");
            //Cloning of styles isn't supported!
            NUnit.Framework.Assert.AreSame(paragraphClone2.ParagraphStyle, paragraphClone.ParagraphStyle, "Must be same style object. Styles have to be cloned explicitly.");
            document.Content.Add(paragraph);
